...
# survive the workbook growing: the SUCs and OEOs sheets already exist but are
# missing PROGRAM and PAP, so they are skipped today and picked up with no code
# change on the day those labels are filled in. Sheets that have figures but
# lack identifiers are reported in the sidebar as pending rather than ignored
# silently.
#
...
<li><strong>Not yet included:</strong> Other Executive Offices (OEOs) and State
Universities and Colleges (SUCs). Philippine Science High School System under
DOST is incomplete.</li>
...
<li><strong>%s unlabeled P/A/Ps are excluded</strong>, and a further %s rows that
